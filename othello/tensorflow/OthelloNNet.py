...
class OthelloNNet:
    def __init__(self, game, args):
        ...
        with self.graph.as_default():
            ...
            h_conv1 = Relu(
                BatchNormalization(
                    self.conv2d(x_image, args.num_channels, "same"), axis=3
                )
            )  # batch_size  x board_x x board_y x num_channels
            h_conv2 = Relu(
                BatchNormalization(
                    self.conv2d(h_conv1, args.num_channels, "same"), axis=3
                )
            )  # batch_size  x board_x x board_y x num_channels
            h_conv3 = Relu(
                BatchNormalization(
                    self.conv2d(h_conv2, args.num_channels, "valid"), axis=3
                )
            )  # batch_size  x (board_x-2) x (board_y-2) x num_channels
            h_conv4 = Relu(
                BatchNormalization(
                    self.conv2d(h_conv3, args.num_channels, "valid"), axis=3
                )
            ...
            s_fc1 = Dropout(
                Relu(BatchNormalization(Dense(h_conv4_flat, 1024), axis=1)),
                rate=self.dropout,
            )  # batch_size x 1024
            s_fc2 = Dropout(
                Relu(BatchNormalization(Dense(s_fc1, 512), axis=1)), rate=self.dropout
            )  # batch_size x 512
